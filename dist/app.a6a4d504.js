...
var expect = _chai.default.expect;
{
  ...
  var useElement = vm.$el.querySelector('use'); // 在vm实例中的element中找use

  var href = useElement.getAttribute('xlink:href'); //找use元素的 xlink:href 是不是存在
...
}
{
  ...
  var _Constructor2 = _vue.default.extend(_button.default); //把Button变成一个构造函数
  ...
  _vm2.$mount(div); //挂载到div里面


  var svg = _vm2.$el.querySelector('svg'); //在vm实例中的element中找svg


  var _window$getComputedSt = window.getComputedStyle(svg),
      order = _window$getComputedSt.order; //解析svg里面的order，CSS样式
...
}
{
  ...
  document.body.appendChild(_div); //在body里面添加一个div

  var _Constructor3 = _vue.default.extend(_button.default); //把Button变成一个构造函数
  ...
  _vm3.$mount(_div); //挂载到div里面


  var _svg = _vm3.$el.querySelector('svg'); //在vm实例中的element中找svg


  var _window$getComputedSt2 = window.getComputedStyle(_svg),
      _order = _window$getComputedSt2.order; //解析svg里面的order，CSS样式


  expect(_order).to.eq('2'); //希望 order 等于 '2'
...
}
{
  ...
  var spy = _chai.default.spy(function () {}); //spy是监听一个函数


  _vm4.$on('click', spy); //当vm被点击的时候，监听函数
